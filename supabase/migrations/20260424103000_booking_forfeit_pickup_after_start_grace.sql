-- Reserva caduca se não houver retirada no horário: após start_date + 15 min (ou fim do período) → no_show e liberta agenda.
-- Manter os 15 min alinhados com BOOKING_PICKUP_GRACE_MINUTES_AFTER_START no frontend.
...
BEGIN
  ...
  WHERE status = 'pending'
    ...
    AND (
      end_date < NOW()
      OR (start_date + INTERVAL '15 minutes') < NOW()
    )
    AND (
...
COMMENT ON FUNCTION public.log_archive_expired_pending_bookings() IS
  'Move reservas pending para no_show: fim do período passado OU não iniciada retirada até 15 min após start_date (equipamento liberto para nova reserva).';
...
BEGIN
    ...
    IF (v_booking.start_date + INTERVAL '15 minutes') < NOW() THEN
        RAISE EXCEPTION 'O prazo para iniciar a retirada nesta reserva expirou (até 15 min após o horário de início).';
    END IF;
